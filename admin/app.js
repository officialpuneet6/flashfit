...
// ----------------------------------------------------
// CATALOG MANAGER — dynamic categories + per-category attributes
// Backed by catalog_categories / category_attributes (RLS: public read
// ACTIVE/enabled, admin write via is_platform_admin()). Degrades to a real
// "not configured" state if the tables are not provisioned yet (§75 — no mock).
// ----------------------------------------------------

let catalogState = { categories: [], attributes: [], selectedCategoryId: null };

const CATALOG_INPUT_TYPES = ["text", "number", "dropdown", "multi-select", "color", "size", "checkbox", "toggle", "date", "image", "url", "rich_text"];
const CAT_INP = "w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-brand-500/40";
const CAT_LBL = "block text-xs font-bold text-slate-500 mb-1";
const CAT_BTN_GHOST = "px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-xs font-bold cursor-pointer";
const CAT_BTN_PRIMARY = "px-4 py-2 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-xs font-bold cursor-pointer disabled:opacity-50";

function flashSlugify(value) {
  return String(value || "").toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
function flashKeyify(value) {
  return String(value || "").toLowerCase().trim().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
}
function catVal(id) { const el = document.getElementById(id); return el ? el.value : ""; }
function catChk(id) { const el = document.getElementById(id); return !!(el && el.checked); }
function catBySort(a, b) {
  return (Number(a.sort_order ?? 100) - Number(b.sort_order ?? 100)) || String(a.name || "").localeCompare(String(b.name || ""));
}

function catalogErrorMessage(err, kind) {
  const code = err && (err.code || err.status);
  const msg = err && err.message ? String(err.message) : "";
  const low = msg.toLowerCase();
  if (code === "23505" || low.includes("duplicate key")) {
    return kind === "attribute" ? "An attribute with this key already exists in this category." : "That slug is already used by another category.";
  }
  if (code === "23503" || low.includes("foreign key")) {
    return "Cannot delete this — sub-categories or linked records still reference it. Archive it, or remove the children first.";
  }
  if (code === "42501" || code === "PGRST301" || low.includes("row-level security") || low.includes("permission denied")) {
    return "Your account is not authorised to manage the catalog. An admin role must be granted first.";
  }
  if (code === "42P01" || code === "PGRST205" || code === "PGRST204" || low.includes("does not exist") || low.includes("could not find") || low.includes("schema cache")) {
    return "Catalog tables are not set up yet. Apply migration 20260902_dynamic_catalog_attributes.sql, then reload.";
  }
  return msg || "Something went wrong. Please try again.";
}

function categoryPathLabel(cat) {
  const parent = cat.parent_id != null ? catalogState.categories.find((x) => Number(x.id) === Number(cat.parent_id)) : null;
...
function collectDescendantIds(id) {
  ...
  while (stack.length) {
    const cur = stack.pop();
    catalogState.categories.forEach((c) => {
      if (c.parent_id != null && Number(c.parent_id) === cur) { out.push(Number(c.id)); stack.push(Number(c.id)); }
    });
  }
  return out;
}

// Fetch + render both panels. Called from the loadAdminData render pipeline.
async function loadCatalogAdmin() {
  const catRoot = document.getElementById("admin-categories-root");
  if (!supabase || !catRoot) return;
  try {
    ...
    if (cErr) throw cErr;
    catalogState.categories = cData || [];
    catalogState.attributes = aErr ? [] : (aData || []);
    if (catalogState.selectedCategoryId && !catalogState.categories.some((c) => Number(c.id) === Number(catalogState.selectedCategoryId))) {
      catalogState.selectedCategoryId = null;
    }
    renderCatalogCategories();
    renderCatalogAttributes();
  } catch (err) {
    renderCatalogUnavailable(err);
  }
}

function renderCatalogUnavailable(err) {
  const message = catalogErrorMessage(err, "load");
  const box = (extra) => `<div class="text-sm text-slate-500 border border-dashed border-amber-300/70 dark:border-amber-500/30 bg-amber-50/60 dark:bg-amber-500/5 rounded-xl p-6">
      <p class="font-bold text-slate-700 dark:text-slate-200 mb-1">Catalog backend not configured yet</p>
      <p>${escapeHtml(message)}</p>${extra || ""}</div>`;
  const catRoot = document.getElementById("admin-categories-root");
  if (catRoot) catRoot.innerHTML = box('<p class="mt-2 text-xs text-slate-400">Once the migration is applied and your account has the admin role, categories will load here.</p>');
  const attrRoot = document.getElementById("admin-attributes-root");
  if (attrRoot) attrRoot.innerHTML = box();
  const newBtn = document.getElementById("admin-attribute-new-btn");
  if (newBtn) newBtn.classList.add("hidden");
}

function categoryRowHtml(cat, depth) {
  const status = cat.status || "ACTIVE";
  const isActive = status === "ACTIVE";
  return `
    <div class="flex items-center justify-between gap-3 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 bg-white dark:bg-slate-900" style="margin-left:${depth * 16}px">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <span class="font-semibold text-sm text-slate-800 dark:text-slate-100 truncate">${escapeHtml(cat.name)}</span>
          <span class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500">${escapeHtml(cat.slug)}</span>
          <span class="text-[10px] font-bold px-1.5 py-0.5 rounded ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-500"}">${isActive ? "ACTIVE" : "INACTIVE"}</span>
        </div>
        <div class="text-[11px] text-slate-400 mt-0.5">sort ${cat.sort_order ?? 100}${cat.description ? " · " + escapeHtml(cat.description) : ""}</div>
      </div>
      <div class="flex items-center gap-1 shrink-0">
        <button onclick="openCategoryForm(null, ${cat.id})" title="Add sub-category" class="text-xs px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 cursor-pointer">+ Sub</button>
        <button onclick="openCategoryForm(${cat.id})" title="Edit" class="text-xs px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 cursor-pointer">Edit</button>
        <button onclick="toggleCategoryStatus(${cat.id})" class="text-xs px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 cursor-pointer">${isActive ? "Archive" : "Activate"}</button>
        <button onclick="deleteCategory(${cat.id})" title="Delete" class="text-xs px-2 py-1 rounded-lg bg-rose-100 text-rose-600 cursor-pointer">Del</button>
      </div>
    </div>`;
}
...
function renderCatalogCategories() {
  ...
  if (!root) return;
  const cats = catalogState.categories;
  if (!cats.length) {
    root.innerHTML = `<div class="text-sm text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-6 text-center">No categories yet. Click <b>+ New Category</b> to create your first one.</div>`;
    return;
  }
  ...
  roots.sort(catBySort);
  let html = "";
  const walk = (cat, depth) => {
    html += categoryRowHtml(cat, depth);
    (childrenOf.get(Number(cat.id)) || []).sort(catBySort).forEach((kid) => walk(kid, depth + 1));
  };
  roots.forEach((r) => walk(r, 0));
  root.innerHTML = `<div class="flex flex-col gap-2">${html}</div>`;
  if (window.lucide) lucide.createIcons();
}

function attributeRowHtml(attr) {
  const enabled = attr.is_enabled !== false;
  const flags = [];
  if (attr.is_required) flags.push("required");
  if (attr.is_searchable) flags.push("searchable");
  if (attr.is_filterable) flags.push("filterable");
  const opts = Array.isArray(attr.options) ? attr.options : [];
  return `
    <div class="flex items-center justify-between gap-3 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 bg-white dark:bg-slate-900">
      <div class="min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <span class="font-semibold text-sm text-slate-800 dark:text-slate-100 truncate">${escapeHtml(attr.name)}</span>
          <span class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500">${escapeHtml(attr.attribute_key)}</span>
          <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700">${escapeHtml(attr.input_type)}</span>
          ${enabled ? "" : '<span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-slate-200 text-slate-500">DISABLED</span>'}
        </div>
        <div class="text-[11px] text-slate-400 mt-0.5">sort ${attr.sort_order ?? 100}${flags.length ? " · " + flags.join(", ") : ""}${opts.length ? " · [" + opts.map((o) => escapeHtml(o)).join(", ") + "]" : ""}</div>
      </div>
      <div class="flex items-center gap-1 shrink-0">
        <button onclick="openAttributeForm(${attr.id})" class="text-xs px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 cursor-pointer">Edit</button>
        <button onclick="toggleAttributeEnabled(${attr.id})" class="text-xs px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 cursor-pointer">${enabled ? "Disable" : "Enable"}</button>
        <button onclick="deleteAttribute(${attr.id})" class="text-xs px-2 py-1 rounded-lg bg-rose-100 text-rose-600 cursor-pointer">Del</button>
      </div>
    </div>`;
}

function renderCatalogAttributes() {
  const root = document.getElementById("admin-attributes-root");
  const newBtn = document.getElementById("admin-attribute-new-btn");
  if (!root) return;
  const cats = catalogState.categories.slice().sort((a, b) => String(categoryPathLabel(a)).localeCompare(String(categoryPathLabel(b))));
  if (!cats.length) {
    root.innerHTML = `<div class="text-sm text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-6 text-center">Create a category first (in the Categories tab), then define its attributes here.</div>`;
    if (newBtn) newBtn.classList.add("hidden");
    return;
  }
  const options = cats.map((c) => `<option value="${c.id}" ${Number(c.id) === Number(catalogState.selectedCategoryId) ? "selected" : ""}>${escapeHtml(categoryPathLabel(c))}</option>`).join("");
  let html = `<div class="flex items-center gap-2 mb-3">
      <label class="text-xs font-bold text-slate-500 shrink-0">Category</label>
      <select id="admin-attr-category-select" onchange="onAttributeCategoryChange(this.value)" class="${CAT_INP}">
        <option value="">Select a category…</option>${options}
      </select>
    </div>`;
  if (!catalogState.selectedCategoryId) {
    html += `<div class="text-sm text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-6 text-center">Pick a category above to view and manage its attributes.</div>`;
    if (newBtn) newBtn.classList.add("hidden");
  } else {
    if (newBtn) newBtn.classList.remove("hidden");
    const attrs = catalogState.attributes.filter((a) => Number(a.category_id) === Number(catalogState.selectedCategoryId)).sort(catBySort);
    html += attrs.length
      ? `<div class="flex flex-col gap-2">${attrs.map(attributeRowHtml).join("")}</div>`
      : `<div class="text-sm text-slate-400 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-6 text-center">No attributes for this category yet. Click <b>+ New Attribute</b> to add one.</div>`;
  }
  root.innerHTML = html;
  if (window.lucide) lucide.createIcons();
}

function onAttributeCategoryChange(value) {
  catalogState.selectedCategoryId = value ? Number(value) : null;
  renderCatalogAttributes();
}

// ---- Modal shell ----
function openCatalogModal(title, subtitle, bodyHtml) {
  const modal = document.getElementById("admin-catalog-modal");
  const t = document.getElementById("admin-catalog-modal-title");
  const s = document.getElementById("admin-catalog-modal-subtitle");
  const b = document.getElementById("admin-catalog-modal-body");
  if (!modal || !b) return;
...
function catAutoSlug(value) {
  const s = document.getElementById("cat-form-slug");
  if (s && s.dataset.touched !== "1") s.value = flashSlugify(value);
}
function attrAutoKey(value) {
  const k = document.getElementById("attr-form-key");
  if (k && k.dataset.touched !== "1") k.value = flashKeyify(value);
}

// ---- Category form + actions ----
function openCategoryForm(id, presetParentId) {
  const editing = id != null && id !== undefined;
  const cat = editing ? catalogState.categories.find((x) => Number(x.id) === Number(id)) : null;
  if (editing && !cat) return showToast("Category not found.", true);
  const excludeIds = new Set();
  if (editing) { excludeIds.add(Number(cat.id)); collectDescendantIds(Number(cat.id)).forEach((x) => excludeIds.add(x)); }
  const parentVal = editing ? (cat.parent_id ?? "") : (presetParentId ?? "");
  const parentOptions = catalogState.categories
    .filter((x) => !excludeIds.has(Number(x.id)))
    .slice().sort((a, b) => String(categoryPathLabel(a)).localeCompare(String(categoryPathLabel(b))))
    .map((x) => `<option value="${x.id}" ${Number(x.id) === Number(parentVal) ? "selected" : ""}>${escapeHtml(categoryPathLabel(x))}</option>`)
    .join("");
  const status = editing ? (cat.status || "ACTIVE") : "ACTIVE";
  const body = `
    <div class="flex flex-col gap-3">
      <div>
        <label class="${CAT_LBL}">Name *</label>
        <input id="cat-form-name" type="text" value="${editing ? escapeHtml(cat.name) : ""}" oninput="catAutoSlug(this.value)" class="${CAT_INP}" placeholder="e.g. Watches">
      </div>
      <div>
        <label class="${CAT_LBL}">Slug *</label>
        <input id="cat-form-slug" type="text" value="${editing ? escapeHtml(cat.slug) : ""}" ${editing ? 'data-touched="1"' : ""} oninput="this.dataset.touched='1'" class="${CAT_INP}" placeholder="watches">
      </div>
      <div>
        <label class="${CAT_LBL}">Parent</label>
        <select id="cat-form-parent" class="${CAT_INP}"><option value="">None (top level)</option>${parentOptions}</select>
      </div>
      <div>
        <label class="${CAT_LBL}">Description</label>
        <textarea id="cat-form-desc" rows="2" class="${CAT_INP}">${editing ? escapeHtml(cat.description || "") : ""}</textarea>
      </div>
      <div>
        <label class="${CAT_LBL}">Image URL</label>
        <input id="cat-form-image" type="url" value="${editing ? escapeHtml(cat.image_url || "") : ""}" class="${CAT_INP}" placeholder="https://…">
      </div>
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="${CAT_LBL}">Status</label>
          <select id="cat-form-status" class="${CAT_INP}">
            <option value="ACTIVE" ${status === "ACTIVE" ? "selected" : ""}>ACTIVE</option>
            <option value="INACTIVE" ${status === "INACTIVE" ? "selected" : ""}>INACTIVE</option>
          </select>
        </div>
        <div>
          <label class="${CAT_LBL}">Sort order</label>
          <input id="cat-form-sort" type="number" value="${editing ? (cat.sort_order ?? 100) : 100}" class="${CAT_INP}">
        </div>
      </div>
      <div class="flex justify-end gap-2 pt-2">
        <button type="button" onclick="closeCatalogModal()" class="${CAT_BTN_GHOST}">Cancel</button>
        <button type="button" id="catalog-modal-save" onclick="saveCategoryForm(${editing ? cat.id : "null"})" class="${CAT_BTN_PRIMARY}">${editing ? "Save changes" : "Create category"}</button>
      </div>
    </div>`;
  openCatalogModal(editing ? "Edit category" : "New category", editing ? escapeHtml(cat.name) : "Add to the taxonomy", body);
}

async function saveCategoryForm(id) {
  const editing = id != null;
  const name = catVal("cat-form-name").trim();
  let slug = catVal("cat-form-slug").trim();
  if (!name) return showToast("Name is required.", true);
  slug = flashSlugify(slug || name);
  if (!slug) return showToast("A valid slug is required.", true);
  const parentRaw = catVal("cat-form-parent");
  const payload = {
    name,
    slug,
    parent_id: parentRaw ? Number(parentRaw) : null,
    description: catVal("cat-form-desc").trim() || null,
    image_url: catVal("cat-form-image").trim() || null,
    status: catVal("cat-form-status") || "ACTIVE",
    sort_order: Number(catVal("cat-form-sort")) || 100
  };
  ...
  try {
    ...
    if (editing) {
      ...
      ({ error } = await supabase.from("catalog_categories").update(payload).eq("id", id));
    } else {
      ({ error } = await supabase.from("catalog_categories").insert(payload));
    }
    if (error) throw error;
    showToast(editing ? "Category updated." : "Category created.");
    await logAdminActivity(editing ? "update_category" : "create_category", "catalog_categories", String(id ?? slug), { slug });
    closeCatalogModal();
    await loadCatalogAdmin();
  } catch (err) {
    showToast(catalogErrorMessage(err, "category"), true);
    if (btn) { btn.disabled = false; btn.textContent = editing ? "Save changes" : "Create category"; }
  }
}

async function toggleCategoryStatus(id) {
  const cat = catalogState.categories.find((x) => Number(x.id) === Number(id));
  if (!cat) return;
  const next = (cat.status || "ACTIVE") === "ACTIVE" ? "INACTIVE" : "ACTIVE";
  const { error } = await supabase.from("catalog_categories").update({ status: next, updated_at: new Date().toISOString() }).eq("id", id);
  if (error) return showToast(catalogErrorMessage(error, "category"), true);
  showToast(`Category ${next === "ACTIVE" ? "activated" : "archived"}.`);
  await logAdminActivity("toggle_category_status", "catalog_categories", String(id), { status: next });
  await loadCatalogAdmin();
}
...
async function deleteCategory(id) {
  ...
  if (!confirm(`Delete category "${label}"? Its attributes are removed and products are detached (their saved category text stays).`)) return;
  const { error } = await supabase.from("catalog_categories").delete().eq("id", id);
  if (error) return showToast(catalogErrorMessage(error, "category-delete"), true);
  if (Number(catalogState.selectedCategoryId) === Number(id)) catalogState.selectedCategoryId = null;
  showToast("Category deleted.");
  await logAdminActivity("delete_category", "catalog_categories", String(id), {});
  await loadCatalogAdmin();
}

// ---- Attribute form + actions ----
function openAttributeForm(id) {
  if (!catalogState.selectedCategoryId) return showToast("Select a category first.", true);
  const editing = id != null && id !== undefined;
  const attr = editing ? catalogState.attributes.find((x) => Number(x.id) === Number(id)) : null;
  if (editing && !attr) return showToast("Attribute not found.", true);
  const cat = catalogState.categories.find((x) => Number(x.id) === Number(catalogState.selectedCategoryId));
  const typeVal = editing ? attr.input_type : "text";
  const typeOptions = CATALOG_INPUT_TYPES.map((t) => `<option value="${t}" ${t === typeVal ? "selected" : ""}>${t}</option>`).join("");
  const opts = editing && Array.isArray(attr.options) ? attr.options.join("\n") : "";
  const chk = (v) => (v ? "checked" : "");
  const body = `
    <div class="flex flex-col gap-3">
      <p class="text-xs text-slate-400">Category: <b class="text-slate-600 dark:text-slate-300">${escapeHtml(cat ? categoryPathLabel(cat) : "-")}</b></p>
      <div>
        <label class="${CAT_LBL}">Field name *</label>
        <input id="attr-form-name" type="text" value="${editing ? escapeHtml(attr.name) : ""}" oninput="attrAutoKey(this.value)" class="${CAT_INP}" placeholder="e.g. Dial Color">
      </div>
      <div>
        <label class="${CAT_LBL}">Attribute key *</label>
        <input id="attr-form-key" type="text" value="${editing ? escapeHtml(attr.attribute_key) : ""}" ${editing ? 'data-touched="1"' : ""} oninput="this.dataset.touched='1'" class="${CAT_INP}" placeholder="dial_color">
      </div>
      <div>
        <label class="${CAT_LBL}">Input type *</label>
        <select id="attr-form-type" class="${CAT_INP}">${typeOptions}</select>
      </div>
      <div>
        <label class="${CAT_LBL}">Options <span class="text-slate-400 font-normal">(one per line — for dropdown / multi-select / size / color presets)</span></label>
        <textarea id="attr-form-options" rows="4" class="${CAT_INP}" placeholder="One option per line">${escapeHtml(opts)}</textarea>
      </div>
      <div class="grid grid-cols-2 gap-2">
        <label class="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><input id="attr-form-required" type="checkbox" ${chk(editing && attr.is_required)}> Required</label>
        <label class="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><input id="attr-form-searchable" type="checkbox" ${chk(editing && attr.is_searchable)}> Searchable</label>
        <label class="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><input id="attr-form-filterable" type="checkbox" ${chk(editing && attr.is_filterable)}> Filterable</label>
        <label class="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-300"><input id="attr-form-enabled" type="checkbox" ${editing ? chk(attr.is_enabled !== false) : "checked"}> Enabled</label>
      </div>
      <div>
        <label class="${CAT_LBL}">Sort order</label>
        <input id="attr-form-sort" type="number" value="${editing ? (attr.sort_order ?? 100) : 100}" class="${CAT_INP}">
      </div>
      <div class="flex justify-end gap-2 pt-2">
        <button type="button" onclick="closeCatalogModal()" class="${CAT_BTN_GHOST}">Cancel</button>
        <button type="button" id="catalog-modal-save" onclick="saveAttributeForm(${editing ? attr.id : "null"})" class="${CAT_BTN_PRIMARY}">${editing ? "Save changes" : "Add attribute"}</button>
      </div>
    </div>`;
  openCatalogModal(editing ? "Edit attribute" : "New attribute", editing ? escapeHtml(attr.name) : "Add a category field", body);
}

async function saveAttributeForm(id) {
  const editing = id != null;
  const category_id = catalogState.selectedCategoryId;
  if (!category_id) return showToast("Select a category first.", true);
  const name = catVal("attr-form-name").trim();
  let key = catVal("attr-form-key").trim();
  if (!name) return showToast("Field name is required.", true);
  key = flashKeyify(key || name);
  if (!key) return showToast("A valid attribute key is required.", true);
  const payload = {
    name,
    attribute_key: key,
    input_type: catVal("attr-form-type") || "text",
    options: catVal("attr-form-options").split(/\r?\n/).map((s) => s.trim()).filter(Boolean),
    is_required: catChk("attr-form-required"),
    is_searchable: catChk("attr-form-searchable"),
    is_filterable: catChk("attr-form-filterable"),
    is_enabled: catChk("attr-form-enabled"),
    sort_order: Number(catVal("attr-form-sort")) || 100
  };
  const btn = document.getElementById("catalog-modal-save");
  if (btn) { btn.disabled = true; btn.textContent = "Saving…"; }
  try {
    let error;
    if (editing) {
      ({ error } = await supabase.from("category_attributes").update(payload).eq("id", id));
    } else {
      payload.category_id = category_id;
      ({ error } = await supabase.from("category_attributes").insert(payload));
    }
    if (error) throw error;
    showToast(editing ? "Attribute updated." : "Attribute added.");
    await logAdminActivity(editing ? "update_attribute" : "create_attribute", "category_attributes", String(id ?? key), { category_id, key });
    closeCatalogModal();
    await loadCatalogAdmin();
  } catch (err) {
    showToast(catalogErrorMessage(err, "attribute"), true);
    if (btn) { btn.disabled = false; btn.textContent = editing ? "Save changes" : "Add attribute"; }
  }
}

async function toggleAttributeEnabled(id) {
  const attr = catalogState.attributes.find((x) => Number(x.id) === Number(id));
  if (!attr) return;
  const currentlyEnabled = attr.is_enabled !== false;
  const { error } = await supabase.from("category_attributes").update({ is_enabled: !currentlyEnabled }).eq("id", id);
  if (error) return showToast(catalogErrorMessage(error, "attribute"), true);
  showToast(currentlyEnabled ? "Attribute disabled." : "Attribute enabled.");
  await logAdminActivity("toggle_attribute", "category_attributes", String(id), { is_enabled: !currentlyEnabled });
  await loadCatalogAdmin();
}

async function deleteAttribute(id) {
  if (!confirm("Delete this attribute? Products keep any values already saved against it.")) return;
  const { error } = await supabase.from("category_attributes").delete().eq("id", id);
  if (error) return showToast(catalogErrorMessage(error, "attribute"), true);
  showToast("Attribute deleted.");
  await logAdminActivity("delete_attribute", "category_attributes", String(id), {});
  await loadCatalogAdmin();
}
...
window.openAttributeForm = openAttributeForm;
window.saveAttributeForm = saveAttributeForm;
window.toggleAttributeEnabled = toggleAttributeEnabled;
window.deleteAttribute = deleteAttribute;
window.onAttributeCategoryChange = onAttributeCategoryChange;
window.closeCatalogModal = closeCatalogModal;
window.catAutoSlug = catAutoSlug;
window.attrAutoKey = attrAutoKey;
