(function () {
  ...
  function roleUrl(role, options = {}) {
    ...
    if (options.url && !/admin-panel|shopfit|delevery_patner|flashfitshop/i.test(options.url)) return options.url;
    return ROLE_URLS[normalized] || "/flashfitshop/index.html";
  }
  ...
  function currentIdentity(explicitRole, explicitRecipientId) {
    ...
    if (!explicitRole) {
      if (path.includes("admin-panel")) role = "admin";
