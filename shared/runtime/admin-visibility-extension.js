(function () {
  ...
  // Resolve the app from the build config/hostname, falling back to the legacy
  // folder name for the rollback copies.
  function isApp(name, legacyFolder) {
    if (window.flashfitApp) return window.flashfitApp.is(name);
    return location.pathname.toLowerCase().includes(legacyFolder);
  }

  function isAdminApp() {
    return isApp("admin", "admin-panel");
  }

  function isCustomerApp() {
    return isApp("customer", "flashfitshop");
  }
