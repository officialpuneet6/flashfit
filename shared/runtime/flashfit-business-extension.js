(function () {
  ...
  function initMode() {
    ...
    activePartner = null;
    // The shared identity module resolves the app from the build config/hostname.
    // The legacy path checks below only apply to the rollback copies, where the
    // folder name is still present in the URL.
    if (window.flashfitApp) mode = window.flashfitApp.role;
    else if (path.includes("delevery_patner")) mode = "delivery_partner";
